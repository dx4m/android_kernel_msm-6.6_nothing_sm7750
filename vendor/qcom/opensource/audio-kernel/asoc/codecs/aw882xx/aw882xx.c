...
#define AW882XX_DRIVER_VERSION "v2.0.5.1"
...
static int aw882xx_mute(struct snd_soc_dai *dai, int mute, int stream)
{
	...
	if (mute) {
		...
		cancel_delayed_work_sync(&aw882xx->stop_work);
		mutex_lock(&aw882xx->lock);
		if (aw882xx->mute_sync)
...
static void aw882xx_stop_work(struct work_struct *work)
{
	struct aw882xx *aw882xx = container_of(work, struct aw882xx, stop_work.work);
...
static int aw882xx_codec_probe(aw_snd_soc_codec_t *aw_codec)
{
	...
	INIT_DELAYED_WORK(&aw882xx->stop_work, aw882xx_stop_work);
...
static int aw882xx_gpio_free(struct aw882xx *aw882xx)
{
	int ret = 0;
	...
	if (gpio_is_valid(aw882xx->reset_gpio)) {
	#ifdef AW_KERNEL_VER_OVER_6_0_0
		gpio_free(aw882xx->reset_gpio);
	#else
		devm_gpio_free(aw882xx->dev, aw882xx->reset_gpio);
	#endif
	}
	...
	#else
		devm_gpio_free(aw882xx->dev, aw882xx->irq_gpio);
	#endif
	}
