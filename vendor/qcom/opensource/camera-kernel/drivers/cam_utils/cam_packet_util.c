	...
	bool exp_mem)
{
	...
	for (i = 0; i < packet->num_patches; i++) {
		...
		if (!temp) {
			CAM_ERR(CAM_UTIL,
				"Reject zero IOVA before patch req %llu patch[%d] dst_hdl 0x%x dst_off 0x%x src_hdl 0x%x src_off 0x%x",
				packet->header.request_id, i,
		...
		if (!(*dst_cpu_addr)) {
			CAM_ERR(CAM_UTIL,
				"Reject zero patched addr req %llu patch[%d] dst_hdl 0x%x dst_off 0x%x src_hdl 0x%x src_off 0x%x base_iova 0x%llx final_iova 0x%llx flags 0x%x exp_mem %d",
				packet->header.request_id, i,
				patch_desc[i].dst_buf_hdl,
				patch_desc[i].dst_offset,
				patch_desc[i].src_buf_hdl,
