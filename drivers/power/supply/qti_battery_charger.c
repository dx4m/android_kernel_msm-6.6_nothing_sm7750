...
static int battery_health_show(struct seq_file *m, void *v)
{
	...
	if (rc < 0)
		return rc;
#ifdef NT_EDIT
	pr_err("end read batt_soh:%d",pst->prop[BATT_SOH]);
#endif
	seq_printf(m, "%d\n", pst->prop[BATT_SOH]);
	return 0;
}
...
static int charge_exist_buck_dump_show(struct seq_file *m, void *v)
{
	struct battery_chg_dev *bcdev = m->private;
	struct psy_state *pst = &bcdev->psy_list[PSY_TYPE_USB];
	int rc = 0;
...
static int charge_exist_buck_dump_open(struct inode *inode, struct file *file)
{
	return single_open(file, charge_exist_buck_dump_show, pde_data(inode));
}
...
const struct nt_proc entries[] = {
	...
	{"charge_exist_buck_dump",{.proc_open = charge_exist_buck_dump_open,
	                  .proc_read = seq_read,
	                  .proc_lseek = seq_lseek,
	                  .proc_release = single_release,}
	},
...
static int battery_chg_parse_dt(struct battery_chg_dev *bcdev)
{
	...
	if (rc < 0) {
		pr_err("previously nt_low-power-off-soc0-vbat:%d\n", bcdev->low_power_off_soc0_vbat);
		bcdev->low_power_off_soc0_vbat = 3300;
		pr_err("current nt_low-power-off-soc0-vbat:%d\n", bcdev->low_power_off_soc0_vbat);
	}
#endif
