...
static inline u8 _ufshcd_wb_get_query_index(struct ufs_hba *hba) {
	if (hba->dev_info.wb_buffer_type == WB_BUF_MODE_LU_DEDICATED)
		return hba->dev_info.wb_dedicated_lu;
	return 0;
}

static inline bool _ufshcd_is_wb_buf_flush_allowed(struct ufs_hba *hba) {
			...
			const char *buf, size_t count)
{
	...
	if (!host->manual_gc.hagc_support) {
		...
		/* flush wb buffer */
		u8 index = _ufshcd_wb_get_query_index(hba);

		ufshcd_query_flag_retry(hba, opcode,
				QUERY_FLAG_IDN_WB_BUFF_FLUSH_DURING_HIBERN8,
				index, NULL);
		if (_ufshcd_is_wb_buf_flush_allowed(hba)) {
			ufshcd_query_flag_retry(hba, opcode,
...
void init_manual_gc(struct ufs_hba *hba)
{
	...
	char wq_name[sizeof("ufs_mgc_hibern8_work")+2+2];
