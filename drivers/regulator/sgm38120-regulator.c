...
static const struct regulator_desc sgm38120_reg[] = {
	SGM38120_DESC(SGM38120_REGULATOR_LDO1, "SGM_L1M", "vin12", 528, 1504, 8,
		     SGM38120_LDO1_VOUT, SGM38120_VSEL_MASK, SGM38120_LDO_EN, BIT(0),
		     BIT(0), 0, sgm38120_crtable1, SGM38120_ILIM, BIT(0), 3),
	SGM38120_DESC(SGM38120_REGULATOR_LDO2, "SGM_L2M", "vin12", 528, 1504, 8,
...
static const struct regmap_range sgm38120_readable_ranges[] = {
	regmap_reg_range(SGM38120_DEVICE_D1, SGM38120_MAX_REG_NO),
};
...
static int sgm38120_regulator_enable_regmap(struct regulator_dev *rdev)
{
	...
	regmap_read(rdev->regmap, SGM38120_LDO_EN, &tmp_data);
	dev_err(dev, "ldo en=%d", tmp_data);
	//regmap_write(rdev->regmap, SGM38120_LDO_EN, ldo6);
	if (ret) {
		...
		//ldo6 = rdev->desc->enable_val | 0x20;
		sgm38120->ldo_en |= val;
		dev_err(dev, "[%s]Set enable val %d", rdev->desc->name, val);
		if(val & 0x20){
			// sgm38120->ldo_en |= 0x80;
			dev_err(dev, "sgm38120 LDO6 is enable,ldo_en = 0x%x,do write again",sgm38120->ldo_en);
			// regmap_write(rdev->regmap, SGM38120_LDO_EN, sgm38120->ldo_en);
		}
	}
	...
	if (NULL != rdev && (debug_en & SGM_DBG_INT_READ))
	{
		...
		// SGM_DBG(dev, "[%s]SGM38120_TSD_UVLO_INT = %x ret = %d", rdev->desc->name,tmp_data ,rc);
		for (i = 0; i < 11; i++){
			ret = regmap_read(sgm38120->regmap, SGM38120_UVP_INT + i, &tmp_data);
			dev_info(dev, "ragAddr = %x regData = %x ret = %d",SGM38120_UVP_INT + i,tmp_data ,ret);
		}
	}
...
static int sgm38120_i2c_probe(struct i2c_client *client)
{
	...
	for (i = 0; i < 11; i++){
		ret = regmap_read(sgm38120->regmap, SGM38120_UVP_INT + i, &tmp_data);
		dev_info(dev, "sgm38120 ragAddr = %x regData = %x ret = %d",SGM38120_UVP_INT + i,tmp_data ,ret);
	}
	...
	regmap_write(sgm38120->regmap, SGM38120_LDO1_VOUT, 0x3E);
	regmap_write(sgm38120->regmap, SGM38120_LDO2_VOUT, 0x57);
	regmap_write(sgm38120->regmap, SGM38120_LDO3_VOUT, 0x57);
	regmap_write(sgm38120->regmap, SGM38120_LDO4_VOUT, 0x25);
	regmap_write(sgm38120->regmap, SGM38120_LDO5_VOUT, 0xC8);
	regmap_write(sgm38120->regmap, SGM38120_LDO6_VOUT, 0xE1);
