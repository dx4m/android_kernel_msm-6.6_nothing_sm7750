...
#include <linux/page_size_compat.h>
#endif
#define HAPTIC_HV_DRIVER_VERSION	"v1.9.0.8"
...
int haptic_hv_i2c_writes(struct aw_haptic *aw_haptic, uint8_t reg_addr, uint8_t *buf, uint32_t len)
{
	...
	mutex_lock(&aw_haptic->i2cwrlock);
	if (aw_haptic->i2c_data == NULL) {
		aw_err("i2c data prt is NULL");
		mutex_unlock(&aw_haptic->i2cwrlock);
		return -ENOMEM;
	}

	if (len > AW_I2C_DATA_LEN) {
		aw_err("i2c master send data len out of range");
		mutex_unlock(&aw_haptic->i2cwrlock);
		return -ERANGE;
	}
	//aw_haptic->i2c_data = kmalloc(AW_I2C_DATA_LEN + 1, GFP_KERNEL);
	memset(aw_haptic->i2c_data, 0, (AW_I2C_DATA_LEN + 1));
	aw_haptic->i2c_data[0] = reg_addr;
	memcpy(&aw_haptic->i2c_data[1], buf, len);
	ret = i2c_master_send(aw_haptic->i2c, aw_haptic->i2c_data, len + 1);
	if (ret < 0)
...
#ifdef AW8693X_LCC_ALGO
void aw_get_sample_params(struct aw_haptic *aw_haptic)
{
	uint8_t restore_d2s_gain = 0;
	aw_info("enter");
	if ((aw_haptic->chipid != AW86936_CHIPID) && (aw_haptic->chipid != AW86937_CHIPID)
		&& (aw_haptic->chipid != AW86938_CHIPID) && (aw_haptic->chipid != AW86937S_CHIPID)
		&& (aw_haptic->chipid != AW86938S_CHIPID))
	{
		aw_info("aw_get_sample_params invalid chipid:%x not support LCC_ALGO", aw_haptic->chipid);
		return;
	}
	restore_d2s_gain = aw_haptic->info.d2s_gain;
	aw_haptic->info.d2s_gain = aw_haptic->info.algo_d2s_gain;
	aw_info("aw_get_sample_params offset_cali");
	aw_haptic->func->offset_cali(aw_haptic);
	aw_info("aw_get_sample_params get_lra_resistance");
	aw_haptic->func->get_lra_resistance(aw_haptic);
	msleep(200);
	aw_info("aw_get_sample_params get_f0");
	aw_haptic->func->upload_lra(aw_haptic, AW_WRITE_ZERO);
	...
	aw_info("aw_get_sample_params get_bemf_peak");
...
int aw_RT_sample_param_judge(struct aw_haptic *aw_haptic)
{
	int i = 0;

	/* peak judge */
	for(i = 0; i < 3; i++) {
		if (aw_haptic->sample_param[i] == 0 || aw_haptic->sample_param[i] == 511)
			return -1;
		if (i < 2) {
			if (aw_haptic->sample_param[i+1] >= aw_haptic->sample_param[i])
				return -1;
		}
	}
	/* f0 judge */
	if (aw_haptic->sample_param[3] > AW_RT_F0_MAX || aw_haptic->sample_param[4] > AW_RT_F0_MAX)
		return -1;
	if (aw_haptic->sample_param[3] < AW_RT_F0_MIN || aw_haptic->sample_param[4] < AW_RT_F0_MIN)
		return -1;
	/* R0 judge */
	if (aw_haptic->sample_param[5] > AW_RT_R0_MAX || aw_haptic->sample_param[5] < AW_RT_R0_MIN)
		return -1;
 ...
 * SMEM region.
 */
#define SMEM_AWINIC_PARAMS		498
/*
 * get uefi params data include f0 f0_cali_data and bemf_peak
 */
static int aw_get_uefi_params_data(struct aw_haptic *aw_haptic)
{
	uint32_t *param_data = NULL;
	uint32_t fitting_f0 = 0;
	size_t buf_size = 0;
	int i = 0;
	int ret = 0;
	...
	aw_haptic->f0 = param_data[0];
	aw_haptic->cont_f0 = param_data[1];
	aw_haptic->f0_cali_data = param_data[2];
	aw_haptic->sample_param[0] = param_data[3];
	aw_haptic->sample_param[1] = param_data[4];
	aw_haptic->sample_param[2] = param_data[5];
	aw_haptic->sample_param[3] = aw_haptic->f0;
	aw_haptic->sample_param[4] = aw_haptic->cont_f0;
	fitting_f0 = (aw_haptic->f0 + aw_haptic->cont_f0) / 2;

	aw_haptic->func->get_lra_resistance(aw_haptic);
	aw_haptic->sample_param[5] = aw_haptic->lra;

	aw_haptic->uefi_param[0] = aw_haptic->sample_param[0];
	aw_haptic->uefi_param[1] = aw_haptic->sample_param[1];
	aw_haptic->uefi_param[2] = aw_haptic->sample_param[2];
	aw_haptic->uefi_param[3] = aw_haptic->sample_param[3];
	aw_haptic->uefi_param[4] = aw_haptic->sample_param[4];
	aw_haptic->uefi_param[5] = aw_haptic->sample_param[5];

	aw_info("haptic_lra_f0 = %u", aw_haptic->f0);
	aw_info("haptic_cont_f0 = %u", aw_haptic->cont_f0);
	aw_info("haptic_fitting_f0 = %u", fitting_f0);
	aw_info("haptic_r0 = %u", aw_haptic->lra);
	aw_info("f0_cali_data = 0x%04x", aw_haptic->f0_cali_data);
	for (i = 0; i < 6; i++)
		aw_info("uefi_sample_param[%d] = 0x%04x", i, aw_haptic->sample_param[i]);

	ret = aw_RT_sample_param_judge(aw_haptic);
	if (ret == 0) {
		aw_haptic->RT_param_update_flag = AW_UPDATE_ENABLE;
	} else {
		aw_get_sample_params(aw_haptic);
		ret = aw_RT_sample_param_judge(aw_haptic);
		if (ret == 0) {
			aw_info("get sample params success");
			aw_haptic->RT_param_update_flag = AW_UPDATE_ENABLE;
		} else {
			aw_info("get sample params failed");
			aw_haptic->RT_param_update_flag = AW_UPDATE_DISABLE;
		}
	}
	return 0;
}
						 ...
						 int file_lens, int rtp_cali_len)
{
	...
	for (i = 0; i < file_lens; i++) {
		...
		for (j = 0; j < seg_dst_len; j++) {
			...
			if (seg_dst_len >= AW_LONG_VIB_POINTS) {
				data_value = (int8_t)(data_value * aw_haptic->rtp_scale_coff[1] * 9 / 10000);
				if (data_value > 127)
					data_value = 127;
				else if (data_value < -127)
					data_value = -127;
			} else {
				data_value = (int8_t)(data_value * aw_haptic->rtp_scale_coff[0] * 9 / 10000);
				if (data_value > 127)
					data_value = 127;
				else if (data_value < -127)
					data_value = -127;
...
static void richtap_update_fifo_data(struct aw_haptic *aw_haptic, uint32_t fifo_len)
{
...
#endif
			if((!atomic_read(&aw_haptic->richtap_rtp_mode)) || (retry <= 0)) {
				atomic_set(&aw_haptic->richtap_rtp_mode, false);
				pr_err("richtap rtp mode false\n");
				return;
			} else {
				retry--;
				pr_info("aac richtap invalid data %d\n", retry);
				usleep_range(1000,1000);
				break;
			}
		}
	...
	pr_info("richtap_update_fifo_data data %d,samples_left = %d\n", pos, samples_left);
}
...
static __always_inline unsigned long nt_calc_vm_flag_bits(unsigned long flags)
{
	unsigned long flag_bits = __nt_calc_vm_flag_bits(flags);
...
static int richtap_file_mmap(struct file *filp, struct vm_area_struct *vma)
{
	...
	vm_flags_t vm_flags = calc_vm_prot_bits(PROT_READ|PROT_WRITE, 0) | nt_calc_vm_flag_bits(MAP_SHARED);
...
{
	struct aw_haptic *aw_haptic = container_of(dev, struct aw_haptic, vib_dev);
	if (aw_haptic->haptic_interface_disable) {
		aw_info("ignore");
		return;
	}
	aw_info("enter");
			      ...
			      const char *buf, size_t count)
{
	...
	if ((aw_haptic->haptic_interface_disable) && (val != 101)) {
		aw_err("%s: val : %d ignore\n", __func__, val);
		return count;
	}
	mutex_lock(&aw_haptic->lock);
	if (!index_init) {
	...
	if (val == 101) {
		aw_info("%s: val: %d activate haptic vib test\n", __func__, val);
		aw_haptic->func->set_repeat_seq(aw_haptic, 4);
		aw_haptic->gain = 0x80;
		aw_haptic->duration = 5000;
	}
...
static ssize_t chip_id_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	cdev_t *cdev = dev_get_drvdata(dev);
	struct aw_haptic *aw_haptic = container_of(cdev, struct aw_haptic, vib_dev);
	char chipid_buf[32] = { 0 };

	switch (aw_haptic->chipid) {
#ifdef AW869X_DRIVER_ENABLE
		case AW8695_CHIPID:
			strcpy(chipid_buf, "aw8695");
			aw_info("detected aw8695.");
			break;
		case AW8697_CHIPID:
			strcpy(chipid_buf, "aw8697");
			aw_info("detected aw8697.");
			break;
#endif
#ifdef AW869XX_DRIVER_ENABLE
		case AW86905_CHIPID:
			strcpy(chipid_buf, "aw86905");
			aw_info("detected aw86905.");
			break;
		case AW86907_CHIPID:
			strcpy(chipid_buf, "aw86907");
			aw_info("detected aw86907.");
			break;
		case AW86915_CHIPID:
			strcpy(chipid_buf, "aw86915");
			aw_info("detected aw86915.");
			break;
		case AW86917_CHIPID:
			strcpy(chipid_buf, "aw86917");
			aw_info("detected aw86917.");
			break;
#endif
#ifdef AW8671X_DRIVER_ENABLE
		case AW86715_CHIPID:
			strcpy(chipid_buf, "aw86715");
			aw_info("detected aw86715.");
			break;
		case AW86716_CHIPID:
			strcpy(chipid_buf, "aw86716");
			aw_info("detected aw86716.");
			break;
		case AW86717_CHIPID:
			strcpy(chipid_buf, "aw86717");
			aw_info("detected aw86717.");
			break;
		case AW86718_CHIPID:
			strcpy(chipid_buf, "aw86718");
			aw_info("detected aw86718.");
			break;
#endif
#ifdef AW8672X_DRIVER_ENABLE
		case AW86727_CHIPID:
			strcpy(chipid_buf, "aw86727");
			aw_info("detected aw86727.");
			break;
		case AW86728_CHIPID:
			strcpy(chipid_buf, "aw86728");
			aw_info("detected aw86728.");
			break;
#endif
#ifdef AW8692X_DRIVER_ENABLE
		case AW86925_CHIPID:
			strcpy(chipid_buf, "aw86925");
			aw_info("detected aw86925.");
			break;
		case AW86926_CHIPID:
			strcpy(chipid_buf, "aw86926");
			aw_info("detected aw86926.");
			break;
		case AW86927_CHIPID:
			strcpy(chipid_buf, "aw86927");
			aw_info("detected aw86927.");
			break;
		case AW86928_CHIPID:
			strcpy(chipid_buf, "aw86928");
			aw_info("detected aw86928.");
			break;
#endif
#ifdef AW8693X_DRIVER_ENABLE
		case AW86936_CHIPID:
			strcpy(chipid_buf, "aw86936");
			aw_info("detected aw86936.");
			break;
		case AW86937_CHIPID:
			strcpy(chipid_buf, "aw86937");
			aw_info("detected aw86937.");
			break;
		case AW86938_CHIPID:
			strcpy(chipid_buf, "aw86938");
			aw_info("detected aw86938.");
			break;
#endif
#ifdef AW8693XS_DRIVER_ENABLE
		case AW86937S_CHIPID:
			strcpy(chipid_buf, "aw86937S");
			aw_info("detected aw86937S.");
			break;
		case AW86938S_CHIPID:
			strcpy(chipid_buf, "aw86938S");
			aw_info("detected aw86938S.");
			break;
#endif
		default:
			strcpy(chipid_buf, "unknown");
			aw_info("unsupport device revision (0x%02X)", aw_haptic->chipid);
			break;
	}
...
#ifdef AW8693X_LCC_ALGO
static ssize_t RT_sample_param_result_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	cdev_t *cdev = dev_get_drvdata(dev);
	struct aw_haptic *aw_haptic = container_of(cdev, struct aw_haptic, vib_dev);

	return snprintf(buf, PAGE_SIZE, "%d\n", aw_haptic->RT_param_update_flag);
}

static ssize_t RT_sample_param_result_store(struct device *dev, struct device_attribute *attr,
				   const char *buf, size_t count)
{
	cdev_t *cdev = dev_get_drvdata(dev);
	struct aw_haptic *aw_haptic = container_of(cdev, struct aw_haptic, vib_dev);
	...
	aw_info("RT_param_update_flag=%d", aw_haptic->RT_param_update_flag);

	return count;
}
static ssize_t glb_state_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	uint8_t glb_state_val = 0;
	cdev_t *cdev = dev_get_drvdata(dev);
	struct aw_haptic *aw_haptic = container_of(cdev, struct aw_haptic, vib_dev);
...
#endif
static ssize_t haptic_interface_disable_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	cdev_t *cdev = dev_get_drvdata(dev);
	struct aw_haptic *aw_haptic = container_of(cdev, struct aw_haptic, vib_dev);

	return snprintf(buf, PAGE_SIZE, "%d\n", aw_haptic->haptic_interface_disable);
}

static ssize_t haptic_interface_disable_store(struct device *dev, struct device_attribute *attr,
				   const char *buf, size_t count)
{
	cdev_t *cdev = dev_get_drvdata(dev);
	struct aw_haptic *aw_haptic = container_of(cdev, struct aw_haptic, vib_dev);
	...
	if (val >= 1)
		aw_haptic->haptic_interface_disable = 1;
	else
		aw_haptic->haptic_interface_disable = 0;

	aw_info("haptic_interface_disable_store=%d", aw_haptic->RT_param_update_flag);
...
#ifdef AW8693X_LCC_ALGO
static DEVICE_ATTR_RW(RT_sample_param_result);
static DEVICE_ATTR_RO(glb_state);
#endif

static DEVICE_ATTR_RW(haptic_interface_disable);
...
#ifdef AW8693X_LCC_ALGO
	&dev_attr_RT_sample_param_result.attr,
	&dev_attr_glb_state.attr,
#endif

	&dev_attr_haptic_interface_disable.attr,
	NULL
};
...
static int haptic_gpp_show(struct seq_file *s, void *unused)
{
	...
	mutex_lock(&g_aw_haptic->lock);
	if (g_aw_haptic->func->get_gpp)
		g_aw_haptic->func->get_gpp(g_aw_haptic);
	mutex_unlock(&g_aw_haptic->lock);
...
static int haptic_lra_resistance_show(struct seq_file *s, void *unused)
{
	...
	mutex_lock(&g_aw_haptic->lock);
	g_aw_haptic->func->get_lra_resistance(g_aw_haptic);
	mutex_unlock(&g_aw_haptic->lock);
...
static int tiktap_file_mmap(struct file *file, struct vm_area_struct *vma)
{
	...
	vm_flags_t vm_flags = calc_vm_prot_bits(PROT_READ|PROT_WRITE, 0) |
			      nt_calc_vm_flag_bits(MAP_SHARED);
...
static int vibrator_init(struct aw_haptic *aw_haptic)
{
	...
	mutex_init(&aw_haptic->i2cwrlock);
...
#ifdef MODULE
static char __haptic_cmdline[2048];
static char *haptic_cmdline = __haptic_cmdline;

const char *haptic_get_cmd(void)
{
    struct device_node *of_chosen = NULL;
    char *bootargs = NULL;

    if (__haptic_cmdline[0] != 0)
        return haptic_cmdline;
    ...
    if (of_chosen) {
        ...
        if (!bootargs)
            aw_info("%s: failed to get bootargs\n", __func__);
        else {
            strcpy(__haptic_cmdline, bootargs);
            aw_info("%s: bootargs: %s\n", __func__, bootargs);
        }
    } else
        aw_info("%s: failed to get /chosen\n", __func__);
...
static int haptic_get_cmdline_f0_data(void)
{
    char *ptr = NULL;
    int f0_data = 0;
    ptr = strstr(haptic_get_cmd(),"haptic.uefi_f0=");
    if(ptr == NULL)
    {
        aw_err("%s get null haptic_get_cmd = %s\n", __func__, haptic_get_cmd());
        return -1;
    }
    pr_info("%s ptr = %s\n", __func__, ptr);
    ptr += strlen("haptic.uefi_f0=");
    f0_data = simple_strtol(ptr, NULL, 10);
    pr_info("%s haptic.uefi_f0 = %d\n", __func__, f0_data);
    return f0_data;
}
...
static int haptic_get_cmdline_cali_data(void)
{
    char *ptr = NULL;
    int aw_cali_data = 0;
    ptr = strstr(haptic_get_cmd(),"haptic.uefi_f0_calidata=");
    if(ptr == NULL)
    {
        aw_err("%s get null haptic_get_cmd = %s\n", __func__, haptic_get_cmd());
        return -1;
    ...
    pr_info("%s ptr = %s\n", __func__, ptr);
    ptr += strlen("haptic.uefi_f0_calidata=");
    aw_cali_data = simple_strtol(ptr, NULL, 10);
    pr_info("%s haptic.uefi_f0_calidata = %d\n", __func__, aw_cali_data);
    return aw_cali_data;
}
...
//	uint8_t reg_val_true=AW8692X_BIT_TMCFG_TM_UNLOCK;
	...
	if (strcmp(uefi_f0_calidata, "NA") == 0) {
		...
		ret = kstrtoint(uefi_f0_calidata,10, &f0_calidata);
	}
	...
	if (0 == strcmp(uefi_f0, "NA")) {
		...
		ret = kstrtoint(uefi_f0,10, &uefi_f0_tmp);
	}

	aw_info("enter");
	if (haptic_get_cmdline_f0_data() > 0)
	{
		uefi_f0_tmp = haptic_get_cmdline_f0_data();
		pr_info("%s uefi_f0_tmp = %d\n", __func__, uefi_f0_tmp);
	}
	if (haptic_get_cmdline_cali_data() > 0)
	{
		f0_calidata = haptic_get_cmdline_cali_data();
		pr_info("%s f0_calidata = %d\n", __func__, f0_calidata);
	}
	/* haptic audio */
	...
	if (abs((int32_t)aw_haptic->f0 - DEFAULT_F0) > F0_THRESHOLD){
		if(abs((int32_t)aw_haptic->f0 - DEFAULT_F0) > F0_THRESHOLD){
			pr_info("%s() cali_f0 %d error use default:%d",
				__func__, aw_haptic->f0, DEFAULT_F0);
			aw_haptic->f0 = DEFAULT_F0;
		} else {
			pr_info("%s() uefi_f0_tmp = %u, aw_haptic->f0 = %u",
				__func__, uefi_f0_tmp, aw_haptic->f0);
		}
	}
#ifdef AW8693X_LCC_ALGO
	aw_get_uefi_params_data(aw_haptic);
	aw_haptic->func->upload_lra(aw_haptic, AW_F0_CALI_LRA);
	pr_info("%s aw_get_sample_params\n", __func__);
#else
	/* f0 calibration */
	//f0_cali(aw_haptic);
#endif
	pr_info("%s set default gain:%d\n", __func__, aw_haptic->gain);
	aw_haptic->haptic_interface_disable = 0;
	mutex_unlock(&aw_haptic->lock);
}
...
static int aw_i2c_probe(struct i2c_client *i2c)
{
	...
	aw_haptic->i2c_data = kmalloc(AW_I2C_DATA_LEN + 1, GFP_KERNEL);
	if(aw_haptic->i2c_data == NULL) {
		pr_err("<%s>%s: malloc rtp memory failed\\n", AW_I2C_NAME, __func__);
		goto err_i2c_data;
	}
	...
	aw_info("aw_haptic->pinctrl state:%d", IS_ERR(aw_haptic->pinctrl_state));
#endif
	if (gpio_is_valid(aw_haptic->irq_gpio)) {
		ret = devm_gpio_request_one(&i2c->dev, aw_haptic->irq_gpio, GPIOF_DIR_IN | GPIOF_INIT_HIGH, "aw_int");
		if (ret) {
			aw_err("int request failed");
			goto err_irq_gpio_request;
		}
	}
	aw_info("irq_gpio state:%d", gpio_get_value(aw_haptic->irq_gpio));
	/* aw func ptr init */
	...
	proc_create_data("f0", 0664, haptic_proc_dir, &haptic_f0_fops, NULL);
	proc_create_data("lra_resistance", 0664, haptic_proc_dir, &haptic_lra_resistance_fops, NULL);
	proc_create_data("gpp", 0664, haptic_proc_dir, &haptic_gpp_fops, NULL);
	aw_info("create proc file success!");
...
err_i2c_data:
	if (aw_haptic->i2c_data)
		kfree(aw_haptic->i2c_data);
	aw_haptic->i2c_data=NULL;
...
#endif
{
	...
	mutex_destroy(&aw_haptic->i2cwrlock);
