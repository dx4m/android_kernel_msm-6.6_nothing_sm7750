	...
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	ics_info("duration exit, val = %d", haptic_data->duration);
	haptic_data->func->resample_ram_waveform(haptic_data);
	ret = haptic_data->func->set_gain(haptic_data, 20);
	haptic_data->chip_config.gain = 20;
	ics_info("gain val = 20\n");
...
static irqreturn_t ics_haptic_irq_handler(int irq, void *data)
{
	...
	if(haptic_data->func->is_error_state(haptic_data))
	{
		if (gpio_is_valid(haptic_data->gpio_en))
		{
			haptic_hw_reset(haptic_data);
			ics_dbg("%s: hardware reset successfully!\n", __func__);
		}
		else
		{
			ics_dbg("%s: hardware reset gpio is NOT valid!\n", __func__);
		}

		ret = haptic_data->func->chip_init(haptic_data, haptic_data->waveform_data, haptic_data->waveform_size);
		ics_dbg("%s: chip reinit successfully!\n", __func__);
		return ret;
	}
